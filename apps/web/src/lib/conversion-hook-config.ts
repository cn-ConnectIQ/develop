...
export function buildHookCopy(
  ...
  if (target === "BAGEVENT") {
    if (context === "event_report_summary") {
      return {
        ...
        body: `这场活动有 ${vars.participantCount ?? 0} 人参会、建立 ${vars.connectionCount ?? 0} 个连接。下次办活动，用百格做报名票务和活动官网，报名数据直接同步到 玖莅，省去手动导入。`,
      };
    }
    return {
      title: "告别反复手动导入名单",
      body: "每次手动导入 Excel 很麻烦？用百格做报名，名单自动同步到 玖莅，活动当天即可现场连接。",
    };
  }
  ...
  if (context === "exhibitor_ai_buyers") {
    return {
      title: "把高意向买家养到成交",
      body: "想对每个高意向买家自动跟进？MarketUP 帮你把线索养到成交，玖莅 采集的数据可一键迁入。",
    };
  }
  ...
  return {
    ...
    body: `你采集了 ${total} 条线索，AI 已评级 A/B/C（A ${a} · B ${b} · C ${c}）。用 MarketUP 自动跟进、再营销并追踪到成交，玖莅 线索可一键迁入。`,
  };
}
